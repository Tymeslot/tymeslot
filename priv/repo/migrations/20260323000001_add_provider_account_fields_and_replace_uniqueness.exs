defmodule Tymeslot.Repo.Migrations.AddProviderAccountFieldsAndReplaceUniqueness do
  ...
  def up do
    ...
    # Backfill CalDAV-family calendar integrations.
    # The canonical formula is "base_url||username", but username is encrypted
    # so we can't access it in SQL. Use base_url alone, then disambiguate
    # duplicates (same user + provider + base_url) by appending '||<id>'.
    #
    # Done in a single UPDATE with a window function so there is no
    # intermediate state that could violate unique indexes.
    execute("""
    ...
    drop_if_exists(
      index(:video_integrations, [:user_id, :provider],
    ...
    create(
      unique_index(:video_integrations, [:user_id, :provider, :provider_account_id],
    ...
    create(
      unique_index(:calendar_integrations, [:user_id, :provider, :provider_account_id],
        where: "is_active = true AND provider_account_id IS NOT NULL",
    ...
    create(
      unique_index(:calendar_integrations, [:user_id, :provider],
        where: "is_active = true AND provider_account_id IS NULL",
