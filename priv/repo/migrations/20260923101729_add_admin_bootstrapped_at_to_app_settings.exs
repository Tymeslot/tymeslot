defmodule Tymeslot.Repo.Migrations.AddAdminBootstrappedAtToAppSettings do
  ...
  def up do
    alter table(:app_settings) do
      add(:admin_bootstrapped_at, :utc_datetime)
    end

    flush()

    # The singleton row is seeded by the create_app_settings migration, but an
    # install could have lost it; the upsert covers that case, and COALESCE
    # keeps any value already present. A single row on a one-row table.
