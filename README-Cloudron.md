...
**A value containing a non-ASCII character must be written with `\uXXXX` escapes inside double quotes.** Write `EMAIL_FROM_NAME="M\u00FCller"`, not `EMAIL_FROM_NAME="Müller"`. The release's parser mishandles raw non-ASCII bytes, and rather than boot with a corrupted value it skips the entry and logs a warning naming the key, so the setting silently falls back to its default. The escaped form is read identically by both readers. This applies to any value, not just display names: an accented character in `SMTP_PASSWORD` or in the password inside `DATABASE_URL` is dropped the same way. `--import` copies such a value across verbatim, so escape it by hand after importing.
