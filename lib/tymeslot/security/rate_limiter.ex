defmodule Tymeslot.Security.RateLimiter do
  ...
  @spec check_calendar_event_edit_rate_limit(integer() | any()) ::
          :ok | {:error, :rate_limited, String.t()} | {:error, :invalid_user_id}
  def check_calendar_event_edit_rate_limit(user_id) when is_integer(user_id) and user_id > 0 do
    check_with_logging(
  ...
  def check_calendar_event_edit_rate_limit(user_id),
    do: invalid_user_id("calendar event edit", user_id)
  ...
  @spec check_calendar_event_move_rate_limit(integer() | any()) ::
          :ok | {:error, :rate_limited, String.t()} | {:error, :invalid_user_id}
  def check_calendar_event_move_rate_limit(user_id) when is_integer(user_id) and user_id > 0 do
    check_multi_bucket_limits([
      {"calendar_event_move:#{user_id}", @event_move_limits, "calendar event move"}
    ])
  end

  def check_calendar_event_move_rate_limit(user_id),
    do: invalid_user_id("calendar event move", user_id)
  ...
  def check_calendar_webhook_rate_limit(id) when is_integer(id) and id > 0,
    do: check_rate_limit("calendar_webhook:#{id}", 60, 60_000)

  def check_calendar_webhook_rate_limit(_id), do: {:error, :rate_limited}
