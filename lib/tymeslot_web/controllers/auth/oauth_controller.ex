defmodule TymeslotWeb.OAuthController do
  @moduledoc """
  Handles OAuth authentication flows for GitHub, Google and generic OAuth.
  """
  ...
  alias Tymeslot.Auth.OAuth.OAuth
  ...
  @doc """
  Initiates Generic OAuth authentication flow.
  """
  ...
  def oauth_auth(conn, _params) do
    case RateLimiter.check_oauth_initiation_rate_limit(ClientIP.get(conn)) do
      :ok ->
        ...
        {updated_conn, authorize_url} = OAuth.authorize_url(conn, redirect_uri)
        redirect(updated_conn, external: authorize_url)
  ...
  @doc """
  Handles Generic OAuth callback.
  """
  ...
  def oauth_callback(conn, _params) do
    ...
    |> put_flash(
      :error,
      "OAuth authentication failed - missing authorization code or security token."
    )
    |> redirect(to: ~p"/?auth=login")
  end
  ...
  defp provider_name(:oauth), do: "OAuth"
