defmodule TymeslotWeb.Components.Icons.IconComponents do
  @moduledoc """
  Brand and product logos with no Heroicon equivalent — Slack, Telegram, Docker,
  n8n, Cloudron and the generic webhook glyph.

  For every other icon use the canonical Heroicon component,
  `<.icon name="hero-…">` (see `TymeslotWeb.Components.CoreComponents.Icons`).
  This module exists only for the handful of marks Heroicons does not ship.
  """
  ...
  def icon(%{name: :slack} = assigns) do
    ~H"""
    <svg class={@class} fill="currentColor" viewBox="0 0 24 24" {@rest}>
  ...
  # Fallback for unknown names — degrade gracefully rather than crash.
  def icon(assigns) do
    ~H"""
    <svg class={@class} fill="none" stroke="currentColor" viewBox="0 0 24 24" {@rest}>
      <path
