defmodule TymeslotWeb.Components.Dashboard.Integrations.Calendar.BaikalConfig do
  @moduledoc """
  Configuration component for Baikal CalDAV integration.
  """
  use TymeslotWeb.Components.Dashboard.Integrations.Calendar.ConfigBase,
    provider: :baikal,
    default_name: "My Baikal"
  ...
  def render(assigns) do
    ~H"""
    <div id={"baikal-config-#{@id}"} class="space-y-6">
      <div class="flex items-start justify-between gap-4 mb-2">
        <div class="flex items-center gap-4">
          <ProviderIcon.provider_icon provider="baikal" type="calendar" size="large" />
          <div>
            <h3 class="text-xl font-black text-tymeslot-900 tracking-tight">Baikal</h3>
            <p class="text-sm text-tymeslot-500 font-medium">
              {dgettext("dashboard_calendar_providers", "PHP-based CalDAV/CardDAV server")}
            </p>
          </div>
        </div>
        <a
          href={docs_guide_url("caldav-baikal")}
          target="_blank"
      ...
      <SharedForm.config_form
        ...
        url_placeholder={
          dgettext("dashboard_calendar_providers", "https://baikal.example.com/dav.php")
        }
      />
    </div>
    """
