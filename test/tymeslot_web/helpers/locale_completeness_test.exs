  ...
  for locale <- @supported_locales do
    describe "#{locale} locale completeness" do
      test "has 12 month names" do
        ...
        assert length(months) == 12, "Expected 12 month names for #{unquote(locale)}, got #{length(months)}"
      end
      ...
      if locale != "en" do
        test "month names differ from English" do
          ...
          assert locale_months != en_months, "#{unquote(locale)} month names should not fall through to English"
        end
      end
      ...
      if locale != "en" do
        test "date format differs from English fallback" do
          ...
          assert locale_format != en_format, "#{unquote(locale)} date format should differ from English"
        end
      end
      ...
      test "time format produces a valid string" do
        ...
        assert is_binary(result) and result != "", "#{unquote(locale)} time format should produce a non-empty string"
      end
      ...
      test "Gettext PO file exists" do
        ...
        assert File.exists?(po_path), "Missing Gettext PO file for #{unquote(locale)} at #{po_path}"
      end
      ...
      test "emails Gettext PO file exists" do
        ...
        assert File.exists?(po_path), "Missing emails Gettext PO file for #{unquote(locale)} at #{po_path}"
      end
    end
  end
end
